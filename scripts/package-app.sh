...
CLI_BINARY_PATH="$BINARY_DIR/$CLI_PRODUCT_NAME"
...
ARTIFACT_BASENAME="Audio-Convolution-Reverb-v${VERSION}-macOS-${ARCH_LABEL}"
...
if [[ -x "$CLI_BINARY_PATH" ]]; then
    cp "$CLI_BINARY_PATH" "$DIST_DIR/${ARTIFACT_BASENAME}-cli"
    chmod +x "$DIST_DIR/${ARTIFACT_BASENAME}-cli"
fi
